...
class CARCCondaSetup:
    ...
    def install_pytorch(self):
        ...
        try:
            ...
            # Install PyTorch using pip as recommended (CUDA 12.6 build)
            install_cmd = f"{init_cmd} && {activate_cmd} && pip install torch torchvision --index-url https://download.pytorch.org/whl/cu126"
    ...
    def install_additional_packages(self):
        ...
        try:
            ...
            # Install wandb via pip (not available on default conda channels)
            print("Installing wandb...")
            wandb_cmd = f"{init_cmd} && {activate_cmd} && pip install wandb"
            subprocess.run(wandb_cmd, shell=True, check=True, executable='/bin/bash')
            self.print_status("Successfully installed wandb", "success")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python carc_conda_setup.py                    # Automated setup with defaults
  python carc_conda_setup.py --interactive     # Interactive setup
  python carc_conda_setup.py --env-name myenv  # Custom environment name
        """
    )
